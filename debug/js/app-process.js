...
// 実行4：MONKEY手番
appProcess.tryMonkey = async function() {
    const {types, player, board} = revCore.data;
    if (types[player] !== 'MONKEY') return false;
    const activeSquares = revMid.getAllActive(board, player);
    const index = Math.trunc(Math.random() * activeSquares.length);
    const pos = activeSquares[index];  // MONKEY指し手取得
    await this.put(pos.x, pos.y);   // 石の配置
    return true;
};
